...
from lab5 import most_common_word, count_words
...
class DatasetAnnotationApp(QtWidgets.QMainWindow):
    ...
    def show_instance(self):
        if self.current_instance:
            ...
            common_word, freq = most_common_word(file_text)
            word_count = count_words(file_text)
            detailed_text = f"Текст отзыва:\n{file_text}"                           

            message_box = QMessageBox(self)
            message_box.setWindowTitle(f"Следующий экземпляр класса {self.class_label}")
            message_box.setText(f"Путь к файлу: {self.current_instance}\nЧисло слов в отзыве: {word_count}\nСамое часто встречающееся слово: '{common_word}' (встречается {freq} раз)")
